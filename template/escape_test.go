...
func TestEscape(t *testing.T) {
	...
	for _, test := range tests {
		...
		input := fmt.Sprintf(`{{define "test"}}%s{{end}}`, test.input)
		set, err = set.Parse(input)
		if err != nil {
		...
		set, err = set.Escape()
		if err != nil {
			t.Errorf("error escaping %q: %v", input, err)
			continue
		}
		b := new(bytes.Buffer)
		if err := set.Execute(b, "test", data); err != nil {
		...
		b.Reset()
		if err := set.Execute(b, "test", pdata); err != nil {
...
func TestEscapeSet(t *testing.T) {
	...
	for _, test := range tests {
		...
		for name, body := range test.inputs {
			input += fmt.Sprintf("{{define %q}}%s{{end}} ", name, body)
		}
		set, err := new(Set).Funcs(fns).Parse(input)
		if err != nil {
			t.Errorf("error parsing %q: %v", input, err)
			continue
		}
		...
		var b bytes.Buffer
		if err := set.Execute(&b, "main", data); err != nil {
			t.Errorf("%q executing %v", err.Error(), set.Tree["main"])
			continue
		}
...
func TestErrors(t *testing.T) {
	tests := []struct {
		...
		{
			"{{range .Items}}<a{{end}}",
			`: on range loop re-entry: "<" in attribute name: "<a"`,
		},
		{
			"\n{{range .Items}} x='<a{{end}}",
			": on range loop re-entry: {{range}} branches",
		},
		{
			"<a b=1 c={{.H}}",
			": ends in a non-text context: {stateAttr delimSpaceOrTagEnd",
		},
		{
			"<script>foo();",
			": ends in a non-text context: {stateJS",
		},
		{
			`<a href="{{if .F}}/foo?a={{else}}/bar/{{end}}{{.H}}">`,
			": {{.H}} appears in an ambiguous URL context",
		},
		{
		...
		{
			`<script>reverseList = [{{template "t"}}]</script>{{end}}` +
				// Missing " after recursive call.
				`{{define "t"}}{{if .Tail}}{{template "t" .Tail}}{{end}}{{.Head}}",`,
	...
	for _, test := range tests {
		...
		input := fmt.Sprintf(`{{define "test"}}%s{{end}}`, test.input)
		set, err := new(Set).Parse(input)
		if err != nil {
			t.Errorf("input=%q: unexpected parse error %s\n", input, err)
			continue
		}
		set, err = set.Escape()
		//err = set.Execute(buf, "test", nil)
		var got string
		if err != nil {
			got = err.Error()
		}
		if test.err == "" {
			if got != "" {
				t.Errorf("input=%q: unexpected error %q", input, got)
			}
			continue
		}
		if strings.Index(got, test.err) == -1 {
			t.Errorf("input=%q\n\terror\n\t%q\ndoes not contain expected string\n\t%q", input, got, test.err)
			continue
		}
	}
}
...
func TestIndirectPrint(t *testing.T) {
	...
	if err != nil {
		...
		err := set.Execute(&buf, "t", ap)
		if buf.String() != "3" {
			t.Errorf(`Expected "3"; got %q`, buf.String())
		}
		buf.Reset()
		err = set.Execute(&buf, "t", bpp)
		if err != nil {
			t.Errorf("Unexpected error: %s", err)
...
func BenchmarkEscapedExecute(b *testing.B) {
	set := Must(new(Set).Parse(`{{define "t"}}<a onclick="alert('{{.}}')">{{.}}</a>{{end}}`))
	var buf bytes.Buffer
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		set.Execute(&buf, "t", "foo & 'bar' & baz")
		buf.Reset()
	}
}
