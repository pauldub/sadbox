...
replacing the "content" block. So let's do it. We  will need a template
that "extends" the previous one. And here is how we declare it:

	{{define "tutorial" "base"}}
	  ...
	{{end}}

See that the {{define}} action has two strings? The first is the name of the
template being defined, and the second is the name of the template being
extended. You can read it as "define tutorial extending base".

A template that extends another one can do only one thing: define blocks
to replace the ones from the base template.
