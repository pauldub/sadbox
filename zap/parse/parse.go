...
// Package zapper zaps templates from text/template and html/template.
package parse
...
type Tree struct {
	...
	Root *DefineNode // top-level root of the tree.
	...
	blocks    map[string]*BlockNode // declared blocks, inlined later.
}
...
func Parse(text, name, leftDelim, rightDelim string, funcs ...map[string]interface{}) (treeSet map[string]*Tree, err error) {
...
func (t *Tree) parse(treeSet map[string]*Tree) (next Node) {
	...
	t.Root = newDefine(1, t.Name, "", list)
	return nil
}

// parseDefinition parses a {{define}} ...  {{end}} template definition and
// installs the definition in the treeSet map.  The "define" keyword has
...
func (t *Tree) parseDefinition(treeSet map[string]*Tree) {
	...
	case itemString, itemRawString:
		parent, err = strconv.Unquote(token.val)
		if err != nil {
			t.error(err)
		}
		if parent == "" {
			t.errorf("parent template name can't be empty in define clause")
		}
		t.expect(itemRightDelim, context)
		list = t.blockList(context)
	default:
		t.unexpected(token, context)
	}
	t.Root = newDefine(lineNum, t.Name, parent, list)
	t.stopParse()
	t.add(treeSet)
}
...
// blockList:
//	{{block}}*
// Terminates at {{end}}. Whitespace is ignored; everything else is an error.
func (t *Tree) blockList(context string) (list *ListNode) {
	list = newList()
	for {
		switch token := t.next(); token.typ {
		case itemEOF:
...
// Block:
//	{{block stringValue}} itemList {{end}}
// Block keyword is past.
func (t *Tree) blockControl() *BlockNode {
	...
	list, end := t.itemList()
	if end.Type() != nodeEnd {
		t.errorf("expected end in %s; found %s", context, end)
	}
	b := newBlock(line, name, list)
	t.addBlock(b)
	return b
}

func (t *Tree) addBlock(b *BlockNode) {
	if t.blocks == nil {
		t.blocks = make(map[string]*BlockNode)
	} else if t.blocks[b.Name] != nil {
		t.errorf("duplicated block name %q", b.Name)
	}
	t.blocks[b.Name] = b
}
